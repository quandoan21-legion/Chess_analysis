...
class Analysis:
    ...
    def most_played_time_format(self):
        if len(self.data) == 0:
            return "No games", 0
        for time_format, count in self.data.time_class.value_counts().items(): 
            return time_format, count
    ...
    def peak_rating(self):
        white_ratings = self.data[self.data.white_username == self.user]["white_rating"]
        black_ratings = self.data[self.data.black_username == self.user]["black_rating"]
        ratings = []
        if len(white_ratings) > 0:
            ratings.append(max(white_ratings))
        if len(black_ratings) > 0:
            ratings.append(max(black_ratings))
    ...
    def highest_white_opp_win(self):
        ...
        if len(data) == 0 or len(data.black_rating) == 0:
            return None
    ...
    def highest_black_opp_win(self):
        ...
        if len(data) == 0 or len(data.white_rating) == 0:
            return None
...
if text and st.button("Collect data and start analysis"):
    ...
    with st.container(border=True):
        ...
        st.markdown(f"##### {text}'s peak rating is :green[{a.peak_rating()}] elo")
         white_win = a.highest_white_opp_win()
         black_win = a.highest_black_opp_win()
         if white_win is not None:
             st.markdown(f"##### With white {text}'s highest rated opponent against whom I won is :blue[{white_win['black_username'].values[0]}] and the opponent's elo was :blue[{white_win['black_rating'].values[0]}]")
         else:
             st.markdown(f"##### With white {text} has no wins to report")
         if black_win is not None:
             st.markdown(f"##### With black it is :blue[{black_win['white_username'].values[0]}] and the opponent's elo was :blue[{black_win['white_rating'].values[0]}]")
         else:
             st.markdown(f"##### With black {text} has no wins to report")
